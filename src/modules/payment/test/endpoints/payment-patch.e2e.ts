...
export function paymentPatchTest(): void {
  ...
  it("should patch only the specified fields", async () => {
    const result = await ctx.paymentConnector.paymentPatch({
      id: payment.id,
      paymentPatch: ctx.paymentPatch,
    });
  ...
  it("should throw a bad request when trying to patch an archived Payment", async () => {
